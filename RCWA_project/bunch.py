...
"""
#    global iteritems = dict.iteritems
#    global iterkeys = dict.iterkeys
...
"""
    if isinstance(x, dict):
        return Bunch( (k, bunchify(v)) for k,v in dict.iteritems(x) )
    elif isinstance(x, (list, tuple)):
        return type(x)( bunchify(v) for v in x )
    else:
        return x
        
        # ajout Patrick 20 02 2013
...
"""
    if isinstance(x, dict):
        return dict( (k, unbunchify(v)) for k,v in dict.iteritems(x) )
    elif isinstance(x, (list, tuple)):
